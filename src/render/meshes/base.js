...
export class Base extends Renderable {
  ...
  _material(options) {
    const precision = this.renderer.getPrecision();
...
`;
    ...
    const material = new THREE.RawShaderMaterial(options);
    for (let key of ["vertexGraph", "fragmentGraph"]) {
  ...
  _raw(object) {
    ...
    return (object.material.defaultAttributeValues = undefined);
  }
